...
const ArtPage = (props) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const artResponse = await fetch(`https://frame-fusion.onrender.com/art/${artId}`);
        const artData = await artResponse.json();
        setArt(artData);

        if (artData) {
          const artistResponse = await fetch(
            `https://frame-fusion.onrender.com/artist/${artData.artistId}`
          );
  ...
  const handleDelete = async () => {
    try {
      const response = await fetch(`https://frame-fusion.onrender.com/art/${artId}`, {
        method: "DELETE",
      });
      navigate("/");
      if (response.ok) {
